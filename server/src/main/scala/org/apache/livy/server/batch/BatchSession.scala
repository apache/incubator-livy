  ...
  def recover(
    ...
    new BatchSession(
      ...
      mockApp.map { m => (_: BatchSession) => m }.getOrElse { s =>
        SparkApp.create(m.appTag, m.appId, None,
          livyConf, Option(s), Map(SparkApp.SPARK_KUBERNETES_NAMESPACE_KEY -> m.namespace))
      })
  }
}
...
class BatchSession(
  ...
  override def infoChanged(appInfo: AppInfo): Unit = {
